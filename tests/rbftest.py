...
class RbfTest(unittest.TestCase):
    ...
    def test_parseTemplate(self):
        ...
        self.assertEqual(self.parser.parseTemplate(), rbf.BoardTemplateParser.ERROR_PARSING_XML)

        #xml template with missing tags
        self.parser.setTemplate("test_templates/incomplete.xml")
        self.assertEqual(self.parser.parseTemplate(), rbf.BoardTemplateParser.ERROR_PARSING_XML_TAGS)
    ...
    def test_createImage(self):
        ...
        self.assertEqual(self.parser.parseTemplate(), 0)
        self.assertEqual(self.parser.createImage(),rbf.BoardTemplateParser.ERROR_IMAGE_SIZE)
        
        #image tag incomplete. no size specified
        self.parser.setTemplate("test_templates/bad_image_tag.xml")
        self.assertEqual(self.parser.parseTemplate(), 0)
        self.assertEqual(self.parser.createImage(),rbf.BoardTemplateParser.ERROR_IMAGE_FILE)
        ...
        testImage.close()
        self.assertEqual(self.parser.createImage(),rbf.BoardTemplateParser.IMAGE_EXISTS)
        os.remove("test.img")
    ...
    def test_createPartitions(self):
        ...
        self.assertEqual(self.parser.parseTemplate(), 0)
        self.assertEqual(self.parser.createImage(), 0)
        self.assertEqual(self.parser.createPartitions(),rbf.BoardTemplateParser.NO_PARTITIONS_FOUND)
        
        #primary partition sizes exceed image size
        self.parser.setTemplate("test_templates/primary_partitions_exceed.xml")
        self.assertEqual(self.parser.parseTemplate(), 0)
        self.assertEqual(self.parser.createImage(), 0)
        self.assertEqual(self.parser.createPartitions(),rbf.BoardTemplateParser.PRIMARY_PARTITION_SIZES_ERROR)
        ...
        self.assertEqual(self.parser.parseTemplate(), 0)
        self.assertEqual(self.parser.createImage(), 0)
        self.assertEqual(self.parser.createPartitions(),rbf.BoardTemplateParser.LOGICAL_PARTITION_SIZES_ERROR)
        
        #incorrect template. has 2 extended partitions
        self.parser.setTemplate("test_templates/two_extended.xml")
        self.assertEqual(self.parser.parseTemplate(), 0)
        self.assertEqual(self.parser.createImage(), 0)
        self.assertEqual(self.parser.createPartitions(),rbf.BoardTemplateParser.INVALID_PARTITION_DATA)
        
        #trying to create logical partition before extended
        self.parser.setTemplate("test_templates/logical_before_extended.xml")
        self.assertEqual(self.parser.parseTemplate(), 0)
        self.assertEqual(self.parser.createImage(), 0)
        self.assertEqual(self.parser.createPartitions(),rbf.BoardTemplateParser.LOGICAL_PART_ERROR)
        
        #total primary partitions > 4
        self.parser.setTemplate("test_templates/five_primary_partitions.xml")
        self.assertEqual(self.parser.parseTemplate(), 0)
        self.assertEqual(self.parser.createImage(), 0)
        self.assertEqual(self.parser.createPartitions(),rbf.BoardTemplateParser.TOTAL_PARTITIONS_ERROR)
    ...
    def test_writeRepos(self):
        ...
        self.assertEqual(self.parser.parseTemplate(), 0)
        self.assertEqual(self.parser.writeRepos(),rbf.BoardTemplateParser.INCORRECT_REPOSITORY)
    ...
    def test_installPackages(self):
        ...
        self.assertEqual(self.parser.parseTemplate(), 0)
        self.assertEqual(self.parser.installPackages(),rbf.BoardTemplateParser.NO_PACKAGES)
    ...
    def test_generatePackageString(self):
        ...
        self.assertEqual(self.parser.generatePackageString(['package1','package2']),"package2 package1 ")
    ...
    def test_installKernel(self):
        ...
        self.assertEqual(self.parser.parseTemplate(), 0)
        self.assertEqual(self.parser.installKernel(),rbf.BoardTemplateParser.NO_UBOOT)
        
        #no kernel type specified
        self.parser.setTemplate("test_templates/no_kernel_type.xml")
        self.assertEqual(self.parser.parseTemplate(), 0)
        self.assertEqual(self.parser.installKernel(),rbf.BoardTemplateParser.NO_KERNEL_TYPE)
        
        #missing firmware
        self.parser.setTemplate("test_templates/bad_firmware.xml")
        self.assertEqual(self.parser.parseTemplate(), 0)
        self.assertEqual(self.parser.installKernel(),rbf.BoardTemplateParser.NO_FIRMWARE_FOUND)
    ...
    def test_finalActions(self):
        ...
        self.assertEqual(self.parser.finalActions(),rbf.BoardTemplateParser.SSH_KEY_NOT_FOUND)
    ...
    def test_makeBootable(self):
        ...
        self.assertEqual(self.parser.parseTemplate(), 0)
        self.assertEqual(self.parser.makeBootable(),rbf.BoardTemplateParser.NO_ETC_OVERLAY)
    ...
    def test_verifyPrimaryPartitionSizes(self):
        ...
        partitionsDom = self.get_partitions("test_templates/partitions_test1.xml")
        self.assertFalse(self.parser.verifyPrimaryPartitionSizes(partitionsDom))
         
        #wrong suffix partition size
        partitionsDom = self.get_partitions("test_templates/partitions_test2.xml")
        self.assertFalse(self.parser.verifyPrimaryPartitionSizes(partitionsDom))
            
        #No root partition
        partitionsDom = self.get_partitions("test_templates/partitions_test3.xml")
        self.assertFalse(self.parser.verifyPrimaryPartitionSizes(partitionsDom))
        
        #primary partition sizes > image size
        partitionsDom = self.get_partitions("test_templates/partitions_test4.xml")
        self.assertFalse(self.parser.verifyPrimaryPartitionSizes(partitionsDom))
    ...
    def test_getTagValue(self):
        ...
        self.parser.setTemplate("test_templates/good.xml")
        self.assertEqual(self.parser.parseTemplate(), 0)
        self.assertEqual(self.parser.getTagValue(self.parser.boardDom,"board"),"cubietruck")
    ...
    def test_configureNetwork(self):
        ...
        self.assertEqual(self.parser.parseTemplate(), 0)
        self.assertEqual(self.parser.configureNetwork(),rbf.BoardTemplateParser.NO_NETWORK)
